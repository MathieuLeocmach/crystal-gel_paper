...
%\title{A novel physical mechanism for the spontaneous formation of porous crystals via viscoelastic phase separation}
\title{\textcolor{red}{Formation of porous crystals via viscoelastic phase separation}} 
...
glass transition~\cite{poon2002,zaccarelli2007,piazza1994phase,verhaegh1997transient,tanaka1999colloid,lu2008gelation} or by crystallization~\cite{soga1999metastable,fortini2008crystallization,perez2011pathways,sabin2012,zhang2012non}. 
...
With a new confocal microscopy protocol, we unveil a novel crystallization pathway to sponge-like porous structures in a colloid-polymer mixture undergoing viscoelastic gas-liquid phase separation.
\textcolor{red}{
We follow the entire kinetics of phase separation from the homogeneous phase to one of its arrested states: at high-polymer concentration this is the gel-phase, but at low-polymer concentration ($c_p\sim 0.38$ mg/g), we find that the liquid phase undergoes a structural rearrangement to a state with higher connectivity, which at high volume fractions ($\phi\sim 0.33$) crystallizes into a porous structure.}
We show that crystallization leads to a network structure different from the original phase separation pattern. Crystals nucleate inside the liquid network, but grow past it by direct condensation of the gas phase on their surface, i.e. de-sublimation, driving evaporation of nearby liquid. %The latter processes via the gas phase has been overlooked so far. %This process represents the colloidal analogue of the Bergeron process~\cite{glickman2000glossary}, which explains the formation of ice crystals in mixed phase clouds and is at the origin of rain. Our finding gives us full experimental access to the kinetic pathway of this poorly understood phenomenon important for climate science. 
...
\begin{figure*}
 \includegraphics{late_structure.pdf}
 \caption{{\bf Structure at late times.} 
{\bf a,} Structure factors for the late stages of the gelation process. The gray and black lines represents respectively random gelation and compact fractal dimensions.
{\bf b,} Late stage of the evolution of the local volume fraction around colloidal particles having 12 nearest neighbours.
{\bf c,} Local volume fraction around a colloidal particle, as a function of the number of nearest neighbours of the particle for the state points in the very late stage. 
...
Samples are shown with the same colour code as in Fig.~\ref{fig:network_formation}. In particular red circles correspond to the crystallizing sample at $\phi=0.33$ and $cp=0.38\,$mg/g.
 }
...
lower volume fraction, which is indeed close to the volume fraction of the attractive glass state~\cite{pham2002multiple}.
\textcolor{red}{These structural analysis provide some evidence that the state point $\phi\approx 0.33$ and $c_p=0.38\,$mg/g could be following a different arrest mechanism, in which phase separation is arrested by crystallization and not by glassiness. In the reminder of this manuscript we investigate this state more closely, to confirm these early results.}
...
\textbf{(Inset)} Time evolution of the average size of the crystals (black line) and the number of crystals (red dashed line) for the same state point.
} 
 \label{fig:crystals}
\end{figure}
...
polymer concentration of all state points. This is shown in the inset of Fig.~\ref{fig:crystals}, where the black line indicates the average size of the crystallites, 
