...
\textcolor{blue}{
We follow the entire kinetics of phase separation, from the homogeneous phase, percolation, stress driven aging, and until the formation of porous crystals.}
...
of crystalline droplets. These droplets originate from crystal seeds that form inside the liquid branches of the spinodal aggregate of network morphology, and then grow with three different growth channels.
...
We show that a necessary condition for the formation of crystal gels is the stress-driven aging of the network, that is possible only at low polymer concentration, and which releases the stresses
that build-up during the network formation due to hydrodynamic interactions, allowing for an increase in the number of average neighbours. If the average concentration of particles is high enough, this relaxation triggers the nucleation process.
...
On the other side, we also observe that increasing the strength of the interparticle attraction
...
At high polymer concentrations (panels \textbf{a}, \textbf{b}), $p_\text{break}$ decreases monotonically, until a stationaty state is reached where $\bar{n}<5$.
At low polymer concentrations (panels \textbf{c}, \textbf{d}), the initial decay of $p_\text{break}$ to a network with $\bar{n}<5$ is followed by a secondary decay to a more compact network with $\bar{n}>6$. The build up of mechanical stress in the first transient network can relax to a more compact network only when when bonds are weak enough, i.e. at low polymer concentrations.
}
...
\subsection{Crystal-growth channels}

While nucleation always occurs inside the liquid branches of the phase-separating network, crystal-growth can proceed through different growth channels.
In the following we will show that the growth mechanism of the crystal is not only due to filling of the spinodal liquid network, as was observed in Refs.~\cite{soga1999metastable,fortini2008crystallization,perez2011pathways} for Brownian Dynamics simulations, but also involves 
The Bergeron process and Ostwald ripening. 
In Fig.~\ref{fig:crystals}b the gyration radius of individual crystalline nuclei for the state point 
...
\textcolor{blue}
{
to allow bond-breaking events that avoid the vitrification of the liquid phase.  
}
...
\begin{figure}
 \centering
 \includegraphics[width=16cm]{./bond_breaking_figure.pdf}
 % bond_breaking_figure.pdf: 1553x1066 pixel, 72dpi, 54.79x37.61 cm, bb=0 0 1553 1066
