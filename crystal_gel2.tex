...
events inside the liquid network (left panel). At the same time liquid regions that have not crystallized start evaporating, and the gas phase contributes to the growth of nuclei (middle panel). Finally the different nuclei coalesce (right panel). \textcolor{red}{Note that the network pattern changes its surface smoothness (??) between $t=2055\,\tau_B$ (left panel) and $t=5577,\tau_B$ (middle panel) (see, e.g., Fig2a): 
not all liquid branches of the network crystallize due to evaporation (?). The crystal-gel structure instead persists, as seen by comparing $t=5577,\tau_B$ (middle panel) and $t=12098,\tau_B$ (right panel). 　
{\bf (Hajime: this part is not so clear for me.)}}}
...
We obtain the best fit between theoretical phase diagram and experimental data for $\sigma=2.24\,\mu$m and $R_g=80$ nm (see Supplementary method). Therefore the polymer-colloid size ratio is $q_R=2R_g/\sigma=0.071$ and the overlap mass fraction of polymer $2.25$~mg/g.
...
\textcolor{blue}{{\bf d,} Late stage of the evolution of local volume fraction around colloidal particles having 12 nearest neighbours.}
 }
...
point $\phi\approx 0.33$ and $c_p=1.07$ mg/g, while the crystal-gel is obtained at the same $\phi$ but
...
\textcolor{red}{\bf Hajime: we need to put length and time information for each panel.} 
}} 
\label{fig:network}
\end{figure*}
...
{\bf a,} Reconstructions from confocal coordinates at $\phi\approx 0.33$ and $c_p=0.38$ mg/g. Depth of view is $5\sigma$. Particles are drawn to scale and coloured according to their phase. Red: crystal; white: liquid; green: gas. \textcolor{red}{\bf Hajime: we need to put length information for images.}
